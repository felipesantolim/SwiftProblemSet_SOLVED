...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs < 10 && music && levels > 5 {
        ...
        if bugs > 10 {
            print("there are a lot of bugs ! Fix")
        }
        
        if !hasMusic {
            print("need fix music")
        }
        
        if levels < 5 {
            print("create more levels")
        }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if !bike && !run {
        print("biking and running are not finished ! you cannnot finish")
        return
    }
    
    if !bike {
        print("biking not finish ! you cannot finish")
    }
    
    if !run {
        print("running not finish ! you cannot finish")
    }
}
